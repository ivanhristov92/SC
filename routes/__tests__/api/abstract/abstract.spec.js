...
describe('Abstract module', () => {
	...
	describe('it should export a "list" function', () => {
		...
		it('should export a "list" function', () => {
			expect(Abstract.list).to.be.a("Function")
		});
		...
		it("should respond to query params and return the 3 latest news", (done)=>{
			...
			.then(resp=>{
				...
				.end((err,res)=>{
					...
					expect(res.body.items[1].title).to.be.eql("fifth");
					expect(res.body.items[2].title).to.be.eql("fourth");
					done();
				})
			...
			.then(resp=>{
					chai.request(server)
						...
						.end((err,res)=>{
							...
							expect(res.body.items[1].title).to.be.eql("fifth");
							expect(res.body.items[2].title).to.be.eql("fourth");
							expect(res.body.items[3].title).to.be.eql("third");
							done();
						})
	...
	describe("it should export a 'getById' function", ()=>{
		...
		it('should export a "getById" function', () => {
			expect(Abstract.getById).to.be.a("Function")
		});
		...
		it('it should get a single piece of news EN', (done) => {
			...
			.then(id=>{
				...
				.end((err, res) => {
					...
					res.body.items[0].title.should.be.eql("New News");
					res.body.items[0].should.have.property("_id");
					res.body.items[0]._id.should.be.eql(id.toString());
					done();
				});	
			})
		});
				...
				.then(id=>{
					chai.request(server)
						...
						.end((err, res) => {
							res.should.have.status(200);
							res.body.items.length.should.be.eql(1);
							res.body.items[0].title.should.be.eql("Zaglavie");
							res.body.items[0].should.have.property("_id");
							res.body.items[0]._id.should.be.eql(id.toString());
							done();
						});
	...
	describe("it should export a 'getBySlug' function", ()=>{
		...
		it('should export a "getBySlug" function', () => {
			expect(Abstract.getBySlug).to.be.a("Function")
		});
